...
void showmovielist(MOVIE& movies, const int movieCount)
{
    ...
    {
        if (idx + 1 < movieCount && movies[idx+1].name == movies[idx].name)
        {
            cout <<
                "    (" << idx  << ") " <<
                std::left << string(3, ' ') <<
...
int member_login(char name[25], char icno[25],char email [40], char phone_number[25],char  acc_num[25], char password[25], char month[25], char year[25], char validity[25])
{
...
void movie_booking()
{
    ...
    if (response =='b'){
        ...
        email[email1.length()] = '\0';   // null terminate
        phone_number1 = phone_input(phone_length);

        for (int j = 0; j < phone_number1.length(); j++){
            phone_number[j] = phone_number1[j];
        }
        phone_number[phone_number1.length()] = '\0';
    }
